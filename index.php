...
if (array_key_exists('address', $_POST) && $data['enabled'] && $data['eligible']) {
    ...
    if ($error) {
        ...
        if (strpos($ret['html'], 'This address is not linked to any account in ExpressCrypto.io')!==false) {
            if (empty($expresscrypto_ref_url)) {
                $expresscrypto_ref_url = 'https://expresscrypto.io/signup?referral=2465';
            }
            $ret['html'] = 'This address is not linked to any account in ExpressCrypto.io. <a href="http://expresscrypto.io/" onmousedown="$(this).attr(\'href\', \''.$expresscrypto_ref_url.'\');" target="_blank">Make an account</a>.';
        }
...
if (($data['service']=='expresscrypto') && ($data['balance']==0)) {
